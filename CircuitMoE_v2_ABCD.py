#!/usr/bin/env python3
"""
Circuit-Specialized MoE v2 for ABCD Classification/Regression.

v2 fixes for quantum experts:
1. poly_coeffs = [0, 1, 0, 0] (single LCU round at init)
2. Uniform mix_coeffs = 1/sqrt(T_pooled)
3. Expert gradient scaling by num_experts
4. Temporal average pooling 363 -> ~36 timesteps

Usage:
    python CircuitMoE_v2_ABCD.py \
        --model-type=quantum --circuit-config=adhd_3 \
...
def get_args():
    ...
    parser.add_argument("--circuit-config", type=str, default="adhd_3",
                        choices=["adhd_3", "adhd_2", "arbitrary_4", "arbitrary_2"],
                        help="Circuit grouping: adhd_3/adhd_2 (neuroscience) or arbitrary_4/arbitrary_2 (index-based)")
    ...
    parser.add_argument("--pool-factor", type=int, default=10,
                        help="Temporal pooling factor for quantum experts (363->36)")
    parser.add_argument("--no-grad-scale", action="store_true", default=False,
                        help="Disable expert gradient scaling")
...
def main():
    ...
    model = CircuitMoE(
        ...
        grad_scale=not args.no_grad_scale,
    ).to(device)
    ...
    if args.model_type == "classical":
        ...
        print(f"Quantum: {args.n_qubits}Q, {args.n_ansatz_layers}L, "
              ...
              f"pool={args.pool_factor}, grad_scale={not args.no_grad_scale}")
