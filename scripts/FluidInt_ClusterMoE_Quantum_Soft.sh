...
#SBATCH --output=logs/FluidInt_ClusterMoE_Quantum_Soft_%j.out
#SBATCH --error=logs/FluidInt_ClusterMoE_Quantum_Soft_%j.err
#SBATCH --job-name=FI_CMoE_QS
...
python ClusterInformedMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-FI_ClusterMoE_QS_local}" \
