...
#SBATCH --output=logs/Sex_ClusterMoE_Classical_Soft_%j.out
#SBATCH --error=logs/Sex_ClusterMoE_Classical_Soft_%j.err
#SBATCH --job-name=Sex_CMoE_CS
...
python ClusterInformedMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-Sex_ClusterMoE_CS_local}" \
