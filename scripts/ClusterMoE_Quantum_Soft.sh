...
#SBATCH --output=logs/ClusterMoE_Quantum_Soft_%j.out
#SBATCH --error=logs/ClusterMoE_Quantum_Soft_%j.err
#SBATCH --job-name=CMoE_QS
...
python ClusterInformedMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-ClusterMoE_QS_local}" \
