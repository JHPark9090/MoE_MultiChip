...
#SBATCH --output=logs/QuantumMoE_ABCD_ADHD_%j.out
#SBATCH --error=logs/QuantumMoE_ABCD_ADHD_%j.err
#SBATCH --job-name=QMoE_ABCD
...
python QuantumDistributedEEGMoE.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-QMoE_ABCD_local}" \
