...
#SBATCH --output=logs/FluidInt_ClusterMoE_Classical_Soft_%j.out
#SBATCH --error=logs/FluidInt_ClusterMoE_Classical_Soft_%j.err
#SBATCH --job-name=FI_CMoE_CS
...
python ClusterInformedMoE_ABCD.py \
    ...
    --cluster-file=results/fluidint_coherence_clustering_site_regressed/cluster_assignments.csv \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-FI_ClusterMoE_CS_local}" \
