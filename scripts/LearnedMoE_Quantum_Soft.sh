...
#SBATCH --output=logs/LearnedMoE_Quantum_Soft_%j.out
#SBATCH --error=logs/LearnedMoE_Quantum_Soft_%j.err
#SBATCH --job-name=LMoE_QS
...
python LearnedRoutingMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-LMoE_QS_local}" \
