...
#SBATCH --output=logs/NAME_Classical_FluidInt_%j.out
#SBATCH --error=logs/NAME_Classical_FluidInt_%j.err
#SBATCH --job-name=NA_C_FI
...
python NetworkAwareBaseline_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-NA_C_FluidInt_local}" \
