...
#SBATCH --output=logs/LearnedMoE_Classical_Soft_%j.out
#SBATCH --error=logs/LearnedMoE_Classical_Soft_%j.err
#SBATCH --job-name=LMoE_CS
...
python LearnedRoutingMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-LMoE_CS_local}" \
