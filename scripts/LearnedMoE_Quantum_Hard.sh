...
#SBATCH --output=logs/LearnedMoE_Quantum_Hard_%j.out
#SBATCH --error=logs/LearnedMoE_Quantum_Hard_%j.err
#SBATCH --job-name=LMoE_QH
...
python LearnedRoutingMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-LMoE_QH_local}" \
