...
#SBATCH --output=logs/SingleExpert_Classical_FluidInt_%j.out
#SBATCH --error=logs/SingleExpert_Classical_FluidInt_%j.err
#SBATCH --job-name=SE_C_FI
...
python SingleExpertBaseline_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-SE_C_FluidInt_local}" \
