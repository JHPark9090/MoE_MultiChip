...
#SBATCH --output=logs/ASD_ClusterMoE_Quantum_Hard_%j.out
#SBATCH --error=logs/ASD_ClusterMoE_Quantum_Hard_%j.err
#SBATCH --job-name=ASD_CMoE_QH
...
python ClusterInformedMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-ASD_ClusterMoE_QH_local}" \
