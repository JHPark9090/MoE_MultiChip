...
#SBATCH --output=logs/Sex_ClusterMoE_Quantum_Hard_%j.out
#SBATCH --error=logs/Sex_ClusterMoE_Quantum_Hard_%j.err
#SBATCH --job-name=Sex_CMoE_QH
...
python ClusterInformedMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-Sex_ClusterMoE_QH_local}" \
