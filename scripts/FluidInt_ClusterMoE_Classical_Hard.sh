...
#SBATCH --output=logs/FluidInt_ClusterMoE_Classical_Hard_%j.out
#SBATCH --error=logs/FluidInt_ClusterMoE_Classical_Hard_%j.err
#SBATCH --job-name=FI_CMoE_CH
...
python ClusterInformedMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-FI_ClusterMoE_CH_local}" \
