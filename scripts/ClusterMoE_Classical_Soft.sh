...
#SBATCH --output=logs/ClusterMoE_Classical_Soft_%j.out
#SBATCH --error=logs/ClusterMoE_Classical_Soft_%j.err
#SBATCH --job-name=CMoE_CS
...
python ClusterInformedMoE_ABCD.py \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-ClusterMoE_CS_local}" \
