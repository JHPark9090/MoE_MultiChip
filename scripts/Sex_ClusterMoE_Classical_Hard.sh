...
#SBATCH --output=logs/Sex_ClusterMoE_Classical_Hard_%j.out
#SBATCH --error=logs/Sex_ClusterMoE_Classical_Hard_%j.err
#SBATCH --job-name=Sex_CMoE_CH
...
python ClusterInformedMoE_ABCD.py \
    ...
    --cluster-file=results/sex_coherence_clustering_site_regressed/cluster_assignments.csv \
    ...
    --sample-size=0 \
    --seed=2025 \
    --job-id="${SLURM_JOB_ID:-Sex_ClusterMoE_CH_local}" \
