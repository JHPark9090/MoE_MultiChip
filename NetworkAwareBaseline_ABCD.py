#!/usr/bin/env python3
"""
Network-Aware Multi-Scale Expert (NAME) Baseline for ABCD Classification/Regression.

Improved feature extraction over SingleExpertBaseline_ABCD.py:
  1. Yeo 17-network grouped spatial projection
  2. Multi-scale dilated depthwise-separable convolutions
  3. Functional connectivity fingerprint branch
  4. Attentive (not mean) temporal pooling
...
class NetworkAwareBaseline(nn.Module):
    """Network-aware expert baseline with classifier head."""

    def __init__(self, total_channels, time_points, model_type,
                 d_net=8, d_model=128,
                 # Classical args:
                 nhead=4,
                 # Quantum args:
        ...
        if model_type == "classical":
            from models.NetworkAwareExpert import NetworkAwareClassicalExpert
            self.expert = NetworkAwareClassicalExpert(
                n_rois=total_channels,
                d_net=d_net,
                d_model=d_model,
                n_time=time_points,
                nhead=nhead,
            ...
            from models.NetworkAwareExpert import NetworkAwareQuantumExpert
            self.expert = NetworkAwareQuantumExpert(
                n_rois=total_channels,
                d_net=d_net,
                d_model=d_model,
                n_time=time_points,
...
def get_args():
    parser = argparse.ArgumentParser(
        description="Network-Aware Multi-Scale Expert (NAME) Baseline for ABCD",
    )
    ...
    # Architecture
    parser.add_argument("--d-net", type=int, default=8,
                        help="Per-network projection dimension")
    parser.add_argument("--d-model", type=int, default=128,
                        help="Intermediate feature width")
    ...
    parser.add_argument("--job-id", type=str, default="NAME")
...
def main():
    ...
    # --- Data Loading ---
    print("\n" + "=" * 80)
    print("Loading ABCD fMRI Dataset (NAME Baseline)...")
    print("=" * 80)
    ...
    # --- Model ---
    print("\n" + "=" * 80)
    print(f"Initializing NAME Baseline ({args.model_type.capitalize()})...")
    print("=" * 80)

    model = NetworkAwareBaseline(
    ...
    if args.model_type == "classical":
        print(f"Classical: d_net={args.d_net}, d_model={args.d_model}, "
              f"nhead={args.nhead}")
    else:
        print(f"Quantum: {args.n_qubits}Q, {args.n_ansatz_layers}L, "
              f"D={args.degree}, d_net={args.d_net}, d_model={args.d_model}")
    ...
    ckpt_name = f"NAME_{args.model_type}_{args.job_id}.pt"
    ...
    if args.wandb:
        import wandb
        wandb.init(project="name-baseline", config=vars(args))
    ...
    print(f"Trainable parameters: {n_params:,}")
    print(f"Config: {args.model_type} / NAME / {task_type}")
    print("=" * 80)
