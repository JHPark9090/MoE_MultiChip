...
# Default data root
UKB_DATA_ROOT = "/pscratch/sd/j/junghoon/UKB"

MAX_TIMEPOINTS = 490  # Native UKB timepoints
PARCEL_DIMS = {"HCP": 360, "HCP180": 360, "Schaefer": 400}
...
def load_ukb_fmri(
    ...
    target_phenotype="fluid_intelligence",
    ...
    clip_std=10.0,
    num_workers=0,
    data_root=UKB_DATA_ROOT,
):
    """
    Loads and preprocesses the UK Biobank resting-state fMRI dataset.

    The UKB dataset contains resting-state fMRI with various parcellations.
    Subject IDs are numeric (eid). fMRI time series are center-cropped to
    max_timepoints for uniform length.

    Note: UKB data may not be present on disk yet. This loader will raise
    a clear error if required files are missing.

    Args:
        seed (int): Random seed for reproducibility.
        device (torch.device): The device to move the tensors to.
        batch_size (int): Number of samples per batch.
        parcel_type (str): Parcellation type. Options:
            - "HCP" / "HCP180": HCP-MMP1 parcellation.
            - "Schaefer": Schaefer 400-parcel 17-network parcellation.
        phenotypes_to_include (list of str or None): Phenotype columns to
            append as extra features alongside fMRI data. If None, fMRI only.
        ...
        max_timepoints (int): Maximum number of timepoints. Longer time series
            are center-cropped to this length. Default 490 (native UKB length).
        min_timepoints (int): Minimum number of fMRI timepoints required.
            ...
            are zero-padded to max_timepoints. Default 300.
        ...
        data_root (str): Root directory containing the UKB data.
    ...
    csv_path = os.path.join(data_root, "UKB_phenotype_gps_fluidint.csv")
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"UKB phenotype CSV not found at {csv_path}. "
            "Please ensure UKB data is available."
        )

    phenotypes = pd.read_csv(csv_path)

    # Keep only rows with complete data for selected columns + target
    required_cols = ["eid", target_phenotype] + phenotypes_to_include
    ...
    if valid_mask.sum() == 0:
        raise FileNotFoundError(
            f"No UKB fMRI files found for parcel_type={parcel_type!r} in {data_root}. "
            "Please ensure fMRI .npy files are available."
        )
    ...
    print(f"[UKB] Valid subjects with fMRI ({parcel_type}, T>={min_timepoints}): {len(subject_ids)}")
    print(f"[UKB] Target: {target_phenotype}")
    ...
    print(f"[UKB] Train: {len(train_idx)}, Val: {len(val_idx)}, Test: {len(test_idx)}")

    # --- Step 3: Load fMRI data ---
    all_fmri = []
    for sid in subject_ids:
        fmri = np.load(_get_fmri_path(data_root, sid, parcel_type))  # (T, R)
        # Center-crop to max_timepoints if longer
        if fmri.shape[0] > max_timepoints:
            start = (fmri.shape[0] - max_timepoints) // 2
            fmri = fmri[start:start + max_timepoints]
        # Zero-pad to max_timepoints if shorter
        elif fmri.shape[0] < max_timepoints:
            pad_len = max_timepoints - fmri.shape[0]
    ...
    if normalize:
        ...
        print(f"\n[UKB] Normalization: per-ROI z-score (fMRI), per-feature z-score (phenotypes)")
        print(f"[UKB] Post-norm fMRI train stats: mean={fmri_train.mean():.4f}, std={fmri_train.std():.4f}")
    ...
    if task_type in ("classification", "binary"):
        ...
        print(f"\n[UKB] Training set shape: {X_train.shape}, "
              f"labels: {np.bincount(y_train.astype(int), minlength=n_classes)}")
        print(f"[UKB] Validation set shape: {X_val.shape}, "
              f"labels: {np.bincount(y_val.astype(int), minlength=n_classes)}")
        print(f"[UKB] Test set shape: {X_test.shape}, "
...
def _get_fmri_path(data_root, subject_id, parcel_type):
    ...
    sid = str(subject_id)
    if parcel_type in ("HCP", "HCP180"):
        return os.path.join(data_root, sid, f"hcp_mmp1_{sid}.npy")
    elif parcel_type == "Schaefer":
        return os.path.join(
            data_root, sid, f"schaefer_400Parcels_17Networks_{sid}.npy"
        )
    else:
